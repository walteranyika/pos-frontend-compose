...
class UnitService(private val httpClient: HttpClient,  private val eventBus: AppEventBus) {
    companion object {
    ...
    suspend fun getUnits(): Result<List<ProductUnitResponse>> =
        safeApiCall<List<ProductUnitResponse>>(eventBus) {
    ...
    suspend fun createUnit(request: ProductUnitRequest): Result<ProductUnitResponse> =
        safeApiCall<ProductUnitResponse> (eventBus){
